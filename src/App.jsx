...
const ORGANISATIONS = [
  "Ambassadeurs Tarte Tatin","APE Collège","APE Lamottoise","APEL St Jean Bosco",
  "AS Foot Nouan Lamotte","ASL Badminton","ASL Judo","ASL Pétanque","ASL Rugby",
  "CIS Lamotte","Club de l'Amitié","Club Photo Lamotte","Cœur de Motardes",
  "Cœur de Sologne Collections","Collège","Comité des Fêtes","Donneurs de Sang",
  "Écoles","EHPAD La Campagnarde","GRAHS","IMDS","Les Plumes d'Or","MDA",
  "Randonneurs Lamottois","Satinka","Speed Form","Téléthon","Tennis",
  "Union Musicale","Service Social","Service Entretien","Service Technique",
  "Agent Technique","Administrateur","Mairie",
];

const ROLES = {
  externe:   { label:"Demandeur externe",   color:"#0ea5e9", bg:"#f0f9ff",  canSeeStock:false, canTreat:false, canManageInventaire:false, canManageUsers:false },
  agent:     { label:"Agent technique",     color:"#8b5cf6", bg:"#f5f3ff",  canSeeStock:true,  canTreat:true,  canManageInventaire:false, canManageUsers:false },
  entretien: { label:"Service Entretien",   color:"#10b981", bg:"#ecfdf5",  canSeeStock:true,  canTreat:true,  canManageInventaire:true,  canManageUsers:false },
  technique: { label:"Service Technique",   color:"#06b6d4", bg:"#ecfeff",  canSeeStock:true,  canTreat:true,  canManageInventaire:true,  canManageUsers:false },
  admin:     { label:"Administrateur",      color:"#ef4444", bg:"#fef2f2",  canSeeStock:true,  canTreat:true,  canManageInventaire:true,  canManageUsers:true  },
  mairie:    { label:"Mairie",              color:"#1d4ed8", bg:"#eff6ff",  canSeeStock:true,  canTreat:true,  canManageInventaire:true,  canManageUsers:true  },
};
...
const Input = ({ label, ...props }) => (
  <div>
    {label && <label style={{ fontSize:13, fontWeight:600, color:"#374151", display:"block", marginBottom:5 }}>{label}</label>}
    <input {...props} style={{ width:"100%", padding:"10px 13px", border:"1.5px solid #e2e8f0", borderRadius:9, fontSize:14, fontFamily:"inherit", background:"#f8fafc", outline:"none", ...props.style }} />
  </div>
);
...
const ProgressBar = ({ statut }) => {
  ...
  if (statut === "refuse") return (
    <div style={{ padding:"9px 14px", background:"#fef2f2", borderRadius:10, border:"1px solid #fca5a5", color:"#dc2626", fontWeight:700, fontSize:13 }}>✕ Demande refusée</div>
  );
  return (
...
const AuthScreen = ({ onLogin }) => {
  ...
  const handleLogin = async () => {
    ...
    if (!profil) { setError("Compte non activé. Contactez l'administrateur."); setLoading(false); return; }
    if (!profil.actif) { setError("Votre compte est désactivé. Contactez l'administrateur."); setLoading(false); return; }
    onLogin({ ...data.user, ...profil });
    setLoading(false);
  };

  const handleRegister = async () => {
    if (!nom || !email || !password || !org) { setError("Tous les champs sont obligatoires"); return; }
    if (password.length < 6) { setError("Le mot de passe doit faire au moins 6 caractères"); return; }
    setLoading(true); setError("");
    const { data, error } = await supabase.auth.signUp({ email, password });
    if (error) { setError(error.message); setLoading(false); return; }
    await supabase.from("profils").insert({
      id: data.user.id, email, nom, organisation: org,
      role: "externe", actif: false,
    });
    setSuccess("Compte créé ! Un administrateur va valider votre accès.");
    setLoading(false);
  };

  return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(160deg,#0d1b3e 0%,#1a3a6e 55%,#0d1b3e 100%)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:24, fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <style>{`
      ...
      <div style={{ textAlign:"center", marginBottom:32, animation:"float 3.2s ease-in-out infinite" }}>
        <div style={{ fontSize:52, marginBottom:10 }}>🏛️</div>
        <div style={{ fontWeight:800, fontSize:26, color:"white" }}>Services Techniques</div>
        <div style={{ fontSize:13, color:"#7da4d8", marginTop:5 }}>Portail municipal — Coeur de Sologne</div>
      </div>

      <div style={{ width:"100%", maxWidth:420, background:"rgba(255,255,255,.08)", borderRadius:20, padding:32, backdropFilter:"blur(12px)", border:"1.5px solid rgba(255,255,255,.12)" }}>
        ...
        <div style={{ display:"flex", background:"rgba(0,0,0,.2)", borderRadius:10, padding:4, marginBottom:24 }}>
          {[["login","Se connecter"],["register","Créer un compte"]].map(([m,l])=>(
            <button key={m} onClick={()=>{ setMode(m); setError(""); setSuccess(""); }}
              style={{ flex:1, padding:"8px", borderRadius:8, border:"none", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit", background:mode===m?"white":"transparent", color:mode===m?"#1a2e4a":"#94a3b8", transition:"all .2s" }}>
              {l}
            </button>
          ))}
        </div>

        {error && <div style={{ padding:"10px 14px", background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:8, color:"#dc2626", fontSize:13, fontWeight:600, marginBottom:16 }}>{error}</div>}
        {success && <div style={{ padding:"10px 14px", background:"#ecfdf5", border:"1px solid #86efac", borderRadius:8, color:"#16a34a", fontSize:13, fontWeight:600, marginBottom:16 }}>{success}</div>}

        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          {mode==="register" && (
            <>
              <div>
                <label style={{ fontSize:13, fontWeight:600, color:"#cbd5e1", display:"block", marginBottom:5 }}>Nom / Prénom *</label>
                <input value={nom} onChange={e=>setNom(e.target.value)} placeholder="Votre nom complet"
                  style={{ width:"100%", padding:"11px 14px", border:"1.5px solid rgba(255,255,255,.15)", borderRadius:9, fontSize:14, fontFamily:"inherit", background:"rgba(255,255,255,.08)", color:"white", outline:"none" }} />
              </div>
              <div>
                <label style={{ fontSize:13, fontWeight:600, color:"#cbd5e1", display:"block", marginBottom:5 }}>Organisation *</label>
                <select value={org} onChange={e=>setOrg(e.target.value)}
                  style={{ width:"100%", padding:"11px 14px", border:"1.5px solid rgba(255,255,255,.15)", borderRadius:9, fontSize:14, fontFamily:"inherit", background:"#1e3a6e", color:"white", outline:"none" }}>
          ...
          <div>
            <label style={{ fontSize:13, fontWeight:600, color:"#cbd5e1", display:"block", marginBottom:5 }}>Email *</label>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="votre@email.fr"
              style={{ width:"100%", padding:"11px 14px", border:"1.5px solid rgba(255,255,255,.15)", borderRadius:9, fontSize:14, fontFamily:"inherit", background:"rgba(255,255,255,.08)", color:"white", outline:"none" }} />
          </div>

          <div>
            <label style={{ fontSize:13, fontWeight:600, color:"#cbd5e1", display:"block", marginBottom:5 }}>Mot de passe *</label>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••"
              style={{ width:"100%", padding:"11px 14px", border:"1.5px solid rgba(255,255,255,.15)", borderRadius:9, fontSize:14, fontFamily:"inherit", background:"rgba(255,255,255,.08)", color:"white", outline:"none" }} />
          </div>

          <button onClick={mode==="login"?handleLogin:handleRegister} disabled={loading}
            style={{ padding:"12px", borderRadius:10, background:"#2563eb", color:"white", border:"none", fontWeight:800, fontSize:15, cursor:"pointer", fontFamily:"inherit", opacity:loading?0.7:1, marginTop:4 }}>
            {loading?"⏳ Chargement…":mode==="login"?"Se connecter →":"Créer mon compte →"}
          </button>
        </div>

        {mode==="register" && (
          <div style={{ marginTop:16, padding:"10px 14px", background:"rgba(255,255,255,.06)", borderRadius:8, fontSize:12, color:"#94a3b8", textAlign:"center" }}>
            Après inscription, un administrateur validera votre accès.
          </div>
        )}
      </div>
    </div>
...
// ═══════════════════════════════════════════════════════════════════
// GESTION DES UTILISATEURS (admin/mairie seulement)
// ═══════════════════════════════════════════════════════════════════
const GestionUtilisateurs = ({ currentUser }) => {
  ...
  const updateUser = async (id, changes) => {
    await supabase.from("profils").update(changes).eq("id", id);
    setUsers(prev => prev.map(u => u.id===id ? {...u,...changes} : u));
  };

  const attente = users.filter(u => !u.actif);
  ...
  return (
    <div>
      {attente.length > 0 && (
        <div style={{ marginBottom:24 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:14 }}>
            <h2 style={{ fontWeight:800, fontSize:18 }}>⏳ En attente de validation</h2>
            <span style={{ background:"#ef4444", color:"white", borderRadius:20, padding:"2px 10px", fontSize:12, fontWeight:700 }}>{attente.length}</span>
          </div>
          <Card>
            {attente.map(u => (
              <div key={u.id} style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", borderBottom:"1px solid #f1f5f9" }}>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700, fontSize:14 }}>{u.nom}</div>
                  <div style={{ fontSize:12, color:"#64748b", marginTop:2 }}>{u.email} · {u.organisation}</div>
                </div>
                <select value={u.role} onChange={e=>updateUser(u.id,{role:e.target.value})}
                  style={{ padding:"6px 10px", border:"1.5px solid #e2e8f0", borderRadius:8, fontSize:13, fontFamily:"inherit", background:"#f8fafc" }}>
                  {Object.entries(ROLES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
                </select>
                <button onClick={()=>updateUser(u.id,{actif:true})}
                  style={{ padding:"7px 16px", borderRadius:8, background:"#ecfdf5", color:"#059669", border:"1.5px solid #6ee7b7", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
                  ✓ Valider
                </button>
                <button onClick={()=>updateUser(u.id,{actif:false})}
                  style={{ padding:"7px 16px", borderRadius:8, background:"#fef2f2", color:"#dc2626", border:"1.5px solid #fca5a5", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
                  ✕ Refuser
                </button>
              </div>
            ))}
      ...
      <div>
        <h2 style={{ fontWeight:800, fontSize:18, marginBottom:14 }}>👥 Utilisateurs actifs ({actifs.length})</h2>
        <Card>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 160px 180px 120px 80px", background:"#f8fafc", borderBottom:"2px solid #e2e8f0", padding:"9px 18px" }}>
            {["Nom / Organisation","Email","Rôle","Statut","Action"].map(h=>(
          ...
          {actifs.map(u => (
            <div key={u.id} style={{ display:"grid", gridTemplateColumns:"1fr 160px 180px 120px 80px", padding:"12px 18px", alignItems:"center", borderBottom:"1px solid #f1f5f9" }}>
              <div>
                <div style={{ fontWeight:600, fontSize:14 }}>{u.nom}</div>
                <div style={{ fontSize:12, color:"#64748b" }}>{u.organisation}</div>
              </div>
              <div style={{ fontSize:13, color:"#64748b" }}>{u.email}</div>
              <div>
                <select value={u.role} onChange={e=>updateUser(u.id,{role:e.target.value})}
                  style={{ width:"100%", padding:"5px 8px", border:"1.5px solid #e2e8f0", borderRadius:8, fontSize:12, fontFamily:"inherit", background:"#f8fafc" }}>
                  {Object.entries(ROLES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}
                </select>
              </div>
              <div>
                <Chip color="#059669" bg="#ecfdf5" small>● Actif</Chip>
              </div>
              <div>
                {u.id !== currentUser.id && (
                  <button onClick={()=>updateUser(u.id,{actif:false})}
                    style={{ padding:"5px 10px", borderRadius:8, background:"#fef2f2", color:"#dc2626", border:"1px solid #fca5a5", fontSize:12, cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>
                    Désactiver
                  </button>
                )}
              </div>
...
// ═══════════════════════════════════════════════════════════════════
const Planning = ({ reservations, inventaire, currentUser, role, onUpdate, onNew }) => {
  ...
  const canSeeStock = role.canSeeStock;
  ...
  const getDispoJour = (matId, jour) => {
    ...
    return { dispo:Math.max(0,mat.qte-reserveQte), total:mat.qte };
  };
  ...
  const submitReservation = async () => {
    ...
    const res = {
      id:`r${Date.now()}`, materiel_id:newRes.materielId, materiel_nom:mat?.nom,
      qte:+newRes.qte, demandeur_id:currentUser.id, demandeur:currentUser.nom,
      debut:newRes.debut, fin:newRes.fin, motif:newRes.motif, statut:"en_attente",
    };
    await onNew(res);
    setModalRes(null);
    ...
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div style={{ fontWeight:700, fontSize:13, minWidth:180, textAlign:"center" }}>{fmt(startDay)} → {fmt(addDays(startDay,13))}</div>
          <button onClick={()=>setSemaine(s=>s+1)} style={{ background:"#f1f5f9", border:"1px solid #e2e8f0", borderRadius:8, padding:"6px 12px", cursor:"pointer", fontWeight:700, fontSize:15, fontFamily:"inherit" }}>›</button>
      ...
      {/* Grille disponibilité — uniquement pour agents internes */}
      {canSeeStock ? (
        <>
            ...
            <table style={{ width:"100%", borderCollapse:"collapse", minWidth:900 }}>
              <thead>
                <tr style={{ background:"#f8fafc" }}>
                  ...
                  {days.map((d,i)=>{
                    const isToday=fmt(d)===fmt(today), isWE=d.getDay()===0||d.getDay()===6;
                    return <th key={i} style={{ padding:"6px 4px", textAlign:"center", fontSize:11, fontWeight:700, borderBottom:"2px solid #e2e8f0", minWidth:52, color:isToday?"#2563eb":isWE?"#94a3b8":"#64748b", background:isToday?"#eff6ff":"#f8fafc" }}>
                      <div>{joursNoms[d.getDay()]}</div><div style={{ fontSize:13, fontWeight:800 }}>{d.getDate()}</div><div style={{ fontSize:10 }}>{moisNoms[d.getMonth()]}</div>
                    </th>;
                  })}
                </tr>
              </thead>
                  ...
                  <tr key={mat.id} style={{ borderBottom:"1px solid #f1f5f9" }}>
                    <td style={{ padding:"8px 14px", position:"sticky", left:0, background:"white", zIndex:1, borderRight:"2px solid #e2e8f0" }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                        ...
                        <div><div style={{ fontWeight:700, fontSize:13 }}>{mat.nom}</div><div style={{ fontSize:11, color:"#94a3b8" }}>Total : {mat.qte}</div></div>
                      </div>
                    </td>
                    {days.map((d,i)=>{
                      ...
                      const isWE=d.getDay()===0||d.getDay()===6, isToday=fmt(d)===fmt(today);
                      return <td key={i} style={{ padding:"4px 3px", textAlign:"center", background:isWE?"#f8fafc":isToday?"#fffbeb":"white" }}>
        ...
        <Card style={{ padding:20, marginBottom:24, background:"#fffbeb", border:"1px solid #fde68a" }}>
          <div style={{ fontSize:14, color:"#92400e", fontWeight:600 }}>
            📋 Pour réserver du matériel, utilisez le bouton "+ Réserver du matériel" ci-dessus. La disponibilité sera vérifiée par les services techniques.
          </div>
        </Card>
      )}
      ...
      <h3 style={{ fontWeight:800, fontSize:16, marginBottom:14 }}>
        {role.canTreat ? "Toutes les réservations" : "Mes réservations"}
      </h3>
      <Card>
        {reservations.filter(r=>!role.canTreat?r.demandeur_id===currentUser.id:true).length===0&&(
          <div style={{ textAlign:"center", padding:36, color:"#cbd5e1" }}>Aucune réservation</div>
        )}
        {reservations.filter(r=>!role.canTreat?r.demandeur_id===currentUser.id:true).map(r=>(
          <div key={r.id} style={{ display:"flex", alignItems:"center", gap:14, padding:"13px 16px", borderBottom:"1px solid #f1f5f9" }}>
            ...
            <div style={{ flex:1 }}>
              ...
              {role.canTreat&&<div style={{ fontSize:12, color:"#94a3b8", marginTop:2 }}>👤 {r.demandeur}</div>}
            </div>
            <Chip color={RSC[r.statut]?.c} bg={RSC[r.statut]?.bg}>{RSC[r.statut]?.i} {RSC[r.statut]?.l}</Chip>
            {role.canTreat&&r.statut==="en_attente"&&(
      ...
      {modalRes==="new"&&(
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.45)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={()=>setModalRes(null)}>
          <div style={{ background:"white", borderRadius:16, width:"100%", maxWidth:480, padding:28 }} onClick={e=>e.stopPropagation()}>
            <div style={{ fontWeight:800, fontSize:18, marginBottom:20 }}>📅 Demande de réservation</div>
            <div style={{ display:"grid", gap:14 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div>
                  <label style={{ fontSize:13, fontWeight:700, color:"#374151", display:"block", marginBottom:5 }}>Date de début *</label>
                ...
                <div>
                  <label style={{ fontSize:13, fontWeight:700, color:"#374151", display:"block", marginBottom:5 }}>Date de fin *</label>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, color:"#374151", display:"block", marginBottom:5 }}>Motif / Événement *</label>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", gap:10, marginTop:20 }}>
              ...
              <button onClick={submitReservation} style={{ padding:"9px 22px", borderRadius:9, background:"#2563eb", color:"white", fontWeight:700, cursor:"pointer", border:"none", fontFamily:"inherit" }}>Envoyer la demande</button>
            </div>
          </div>
        </div>
      )}
...
// MODULE INVENTAIRE
// ═══════════════════════════════════════════════════════════════════
const Inventaire = ({ inventaire, onUpdate, onAdd, currentUser, role }) => {
  ...
  return (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        ...
        {role.canManageInventaire && (
          <button onClick={()=>setModal("add")} style={{ background:"#2563eb", color:"white", padding:"8px 18px", borderRadius:9, border:"none", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>+ Ajouter matériel</button>
        )}
      </div>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.45)", zIndex:200, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={()=>setModal(null)}>
          <div style={{ background:"white", borderRadius:16, width:"100%", maxWidth:480, padding:28 }} onClick={e=>e.stopPropagation()}>
            ...
            <div style={{ display:"grid", gap:13 }}>
              ...
              <div>
                <label style={{ fontSize:13, fontWeight:700, color:"#374151", display:"block", marginBottom:4 }}>Réservable par les organismes ?</label>
                <div style={{ display:"flex", gap:10 }}>
                  {[true,false].map(v=>(
...
export default function App() {
  ...
  const role = currentUser ? (ROLES[currentUser.role] || ROLES.externe) : null;
  ...
  const chargerDonnees = async () => {
    ...
    try {
      ...
      if (inv && inv.length > 0) { setInventaire(inv); }
      else { await supabase.from("inventaire").insert(INVENTAIRE_DEFAUT); setInventaire(INVENTAIRE_DEFAUT); }
  ...
  const handleLogout = async () => {
    ...
    setCurrentUser(null);
    setDemandes([]); setInventaire([]); setReservations([]);
  };
  ...
  const submitDemande = async () => {
    ...
    const d = {
      id:`DEM-${Date.now()}`, type:newForm.type, titre:newForm.titre,
      description:newForm.description, service:newForm.service, priorite:newForm.priorite,
      statut:"ouvert", demandeur_id:currentUser.id, technicien_id:null,
      date:fmt(today), localisation:newForm.localisation||null,
      quantite:newForm.quantite?+newForm.quantite:null,
      budget:newForm.budget?+newForm.budget:null, commentaires:[],
    };
    const { error } = await supabase.from("demandes").insert(d);
    if (!error) { setDemandes(prev=>[d,...prev]); setPage("liste"); setNewForm({ type:"intervention", titre:"", description:"", service:"Bâtiments", priorite:"normale", localisation:"", quantite:"", budget:"" }); }
  };
  ...
  const visDemandes = demandes.filter(d => {
    if (!role?.canTreat && d.demandeur_id!==currentUser?.id) return false;
  ...
  const stats = {
    ...
    urgentes:demandes.filter(d=>d.priorite==="urgente"&&d.statut!=="resolu"&&(!role?.canTreat?d.demandeur_id===currentUser?.id:true)).length,
    resaAttente:reservations.filter(r=>r.statut==="en_attente"&&(!role?.canTreat?r.demandeur_id===currentUser?.id:true)).length,
    usersAttente:0,
  };
  ...
  const TABS = [
    ...
    { id:"inventaire",   icon:"📦", label:"Inventaire",  badge:0,             show:true },
    { id:"reservations", icon:"📅", label:"Planning",    badge:stats.resaAttente },
    { id:"utilisateurs", icon:"👥", label:"Utilisateurs",badge:stats.usersAttente, show:role.canManageUsers },
  ];
    ...
    <div style={{ fontFamily:"'DM Sans','Segoe UI',sans-serif", minHeight:"100vh", background:"#f1f5f9", color:"#0f172a" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        input[type=text],input[type=number],input[type=date],input[type=email],input[type=password],select,textarea{width:100%;padding:10px 13px;border:1.5px solid #e2e8f0;border-radius:9px;font-size:14px;color:#0f172a;outline:none;transition:border-color .15s;background:#f8fafc;}
        ...
        <div style={{ maxWidth:1240, margin:"0 auto", display:"flex", alignItems:"center", justifyContent:"space-between", height:56 }}>
          ...
          <div style={{ display:"flex", gap:2 }}>
            {TABS.filter(t=>t.show!==false).map(t=>(
              <button key={t.id} className="btn" onClick={()=>{ setTab(t.id); setPage("liste"); }}
                style={{ padding:"6px 14px", borderRadius:8, fontSize:13, fontWeight:700, background:tab===t.id?"#eff6ff":"transparent", color:tab===t.id?"#2563eb":"#6b7280", position:"relative" }}>
                {t.icon} {t.label}
                {t.badge>0 && <span style={{ position:"absolute", top:2, right:2, width:16, height:16, borderRadius:"50%", background:"#ef4444", color:"white", fontSize:9, fontWeight:800, display:"flex", alignItems:"center", justifyContent:"center" }}>{t.badge}</span>}
              </button>
            ))}
          </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"5px 12px", background:"#f8fafc", border:"1px solid #e2e8f0", borderRadius:20 }}>
              <div style={{ width:28, height:28, borderRadius:"50%", background:role.color, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, color:"white", fontWeight:700 }}>
                {currentUser.nom?.charAt(0)?.toUpperCase()}
              </div>
              <div>
                <div style={{ fontSize:12, fontWeight:700, lineHeight:1.2 }}>{currentUser.nom}</div>
                <div style={{ fontSize:10, color:role.color, fontWeight:700 }}>{role.label}</div>
              </div>
            </div>
            <button className="btn" onClick={handleLogout} style={{ background:"#f1f5f9", color:"#64748b", padding:"6px 12px", borderRadius:8, fontSize:12, fontWeight:700, border:"1px solid #e2e8f0" }}>
              Déconnexion
            </button>
          </div>
        </div>
      ...
      <div style={{ maxWidth:1240, margin:"0 auto", padding:22 }}>
        ...
        {/* UTILISATEURS */}
        {tab==="utilisateurs" && role.canManageUsers && (
          <div className="anim">
            <h1 style={{ fontWeight:800, fontSize:22, marginBottom:18 }}>👥 Gestion des utilisateurs</h1>
            <GestionUtilisateurs currentUser={currentUser} />
          </div>
        )}
        ...
        {tab==="inventaire" && (
          <div className="anim">
            ...
            <Inventaire inventaire={inventaire} onUpdate={updateInventaire} onAdd={addInventaire} currentUser={currentUser} role={role} />
          </div>
        )}
        ...
        {tab==="reservations" && (
          <div className="anim">
            ...
            <Planning reservations={reservations} inventaire={inventaire} currentUser={currentUser} role={role} onUpdate={updateReservation} onNew={addReservation} />
          </div>
        )}
              ...
              <div>
                ...
                <div style={{ background:"white", borderRadius:14, border:"1px solid #e2e8f0", maxWidth:580, padding:28 }}>
                  ...
                  <div style={{ display:"grid", gap:15 }}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                      <div><label>Service concerné</label>
                        <select value={newForm.service} onChange={e=>setNewForm({...newForm,service:e.target.value})}>
                          {SERVICES.map(s=><option key={s}>{s}</option>)}
                        </select>
                      </div>
                      <div><label>Niveau d'urgence</label>
                        <select value={newForm.priorite} onChange={e=>setNewForm({...newForm,priorite:e.target.value})}>
                          {PRIORITES.map(p=><option key={p} value={p}>{PC[p].l}</option>)}
                        </select>
                      </div>
                    </div>
                    {newForm.type==="intervention"
                      ...
                      : <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                          ...
                          <div><label>Budget estimé (€)</label><input type="number" min="0" value={newForm.budget} onChange={e=>setNewForm({...newForm,budget:e.target.value})} /></div>
                        </div>
                    }
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 320px", gap:18, alignItems:"start" }}>
                  <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                    ...
                    <Card style={{ padding:22 }}>
                      ...
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                        {[["Service",selected.service],["Date",selected.date],["Organisation",currentUser.organisation],selected.localisation?["Lieu",selected.localisation]:null].filter(Boolean).map(([k,v])=>(
                          <div key={k}>
                    ...
                    <Card style={{ padding:22 }}>
                      ...
                      {(selected.commentaires||[]).map((c,i)=>(
                        <div key={i} style={{ borderRadius:11, padding:"11px 13px", marginBottom:8, background:c.role==="admin"||c.role==="mairie"?"#fef2f2":c.role==="externe"?"#f0f9ff":"#f5f3ff", border:`1px solid ${c.role==="admin"||c.role==="mairie"?"#fca5a5":c.role==="externe"?"#bae6fd":"#c4b5fd"}` }}>
                          <div style={{ display:"flex", justifyContent:"space-between", marginBottom:5 }}>
                            <span style={{ fontWeight:700, fontSize:13, color:ROLES[c.role]?.color||"#374151" }}>{c.auteur}</span>
                      ...
                      {selected.statut!=="resolu"&&selected.statut!=="refuse"&&(
                        <div style={{ display:"flex", gap:10, marginTop:10 }}>
                          <textarea rows={2} placeholder="Votre commentaire…" value={newComment} onChange={e=>setNewComment(e.target.value)} style={{ flex:1, resize:"none" }} />
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                    <Card style={{ padding:18 }}>
                      ...
                      {role.canTreat && STATUTS_DEM.filter(s=>s!==selected.statut).map(s=>(
            ...
            {page==="liste" && (
              <div>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18 }}>
                  <div>
                    <h1 style={{ fontSize:22, fontWeight:800 }}>{role.canTreat?"Toutes les demandes":"Mes demandes"}</h1>
                    <p style={{ fontSize:13, color:"#64748b", marginTop:3 }}>{currentUser.organisation} · {role.label}</p>
                  </div>
                  <button className="btn" onClick={()=>setPage("nouvelle")} style={{ background:"#2563eb", color:"white", padding:"9px 18px", borderRadius:9, fontSize:13, fontWeight:700 }}>+ Nouvelle demande</button>
                </div>
                ...
                <Card style={{ overflow:"hidden" }}>
                  ...
                  {visDemandes.map(d=>(
                    <div key={d.id} className="row" style={{ display:"grid", gridTemplateColumns:"120px 1fr 130px 95px 105px 85px", padding:"12px 15px", alignItems:"center" }}
                      ...
                      <div style={{ fontFamily:"monospace", fontSize:11, color:"#2563eb", fontWeight:600 }}>{d.id.slice(-8)}</div>
